<?php
    ...
    class Rokudoku{
        protected $ime;
        protected $ime_tablice_za_ucitat;
        ...
        {
            $this->ime = false;
            $this->ime_tablice_za_ucitat = false;
        ...
        function obradi_unos()
        {
            ...
            elseif( $_POST['akcija'] === 'reset' ){
                $name = $this->ime;
                $ime_tablice = $this->ime_tablice_za_ucitat;
                $this->__construct();
                $this->ime = $name;
                $this->load_table($ime_tablice);
                $this->ime_tablice_za_ucitat = $ime_tablice;

                
            }
            elseif( $_POST['akcija'] === 'reset_cijeli' ){
                $this->__construct();
                session_unset();
                session_destroy();
            }
...
//var_dump($this->boje);
        ...
        function izvadi_brojeve()
        {
            $flag = 0;
            $post = $_POST['polje_za_unos'];
            for( $i = 0; $i < 6; ++$i)
                for( $j = 0; $j < 6; ++$j )
                {
                    if( isset($_POST['polje_za_unos'][$i][$j]) )
                    {
                        if( preg_match( '/^[1-6]$/', $_POST['polje_za_unos'][$i][$j] ) )
                        {
                            $flag = $this->unesi_broj( $i, $j, $_POST['polje_za_unos'][$i][$j] );
                        }
                        //else    //krivo unoesen broj
                            //$this->postavi_error(true, "Krivo unos u celiju!");
        ...
        function unesi_broj($red, $stupac, $broj_za_unjet /*= $_POST['broj_za_unjet']*/)
        {   
            $num = 0;
           if( !in_array( $broj_za_unjet, $this->polje_dozvoljenih( $red, $stupac) ) ){    //nije u polju dozvoljenih ofarbat ga crveno i stavit unutra
                $this->polje[$red][$stupac] = $broj_za_unjet;
                $this->boje[$red][$stupac] = -2;
                //$this->broj_pokusaja++;
                $num++;
           }
           elseif( in_array( $broj_za_unjet, $this->polje_dozvoljenih( $red, $stupac) ) ){     //je u polju, ofarbat ga plavo is tavit unutra
                $this->polje[$red][$stupac] = $broj_za_unjet;
        ...
        {
            if( $this->ime !== false && $this->ime_tablice_za_ucitat !== false)
                return $this->ime;
            elseif( isset( $_POST['ime'] ) && isset( $_POST['br_tablice'] ) ){   //salje se ime igrača
                if( preg_match ( '/^[a-zA-Z]{1,20}$/', $_POST['ime'] ) ){ //    ime je dobro
                    $this->ime = $_POST['ime'];
                    $this->ime_tablice_za_ucitat = $_POST['br_tablice'];
                    $this->load_table( $this->ime_tablice_za_ucitat);
                    return $this->ime;
                }
                else{   //  krivo ime uneseno   -   doradit errore
                    $this->postavi_error( true, 'Error: krivo uneseno ime ili  nije odabrana tablica! Try again.');
                    return false;
                }
            }
            else
        ...
        {   
            ...
            <input type="radio" name="akcija" id="reset" value="reset">
                <label for="reset">Želim sve ispočetka!(Samo obrisat unesene brojeve)</label><br><br>
            <!--<input type="radio" name="akcija" id="reset_cijeli" value="reset_cijeli" disabled>
                <label for="reset_cijeli">Želim sve ispočetka!(Ponovo birat tablicu i unosit ime)</label>--><br><br>
        ...
        {   
            ...
            echo '<p>Unutar svake prazne ćelije ispišite malim fontom pomoć igraču – sve brojeve koje smije unijeti u to polje, a da ne prekrši pravilo igre; vidi sliku dolje. </p>';
        }
        ...
        function ispisi_tablicu_iz_filea( $ime_filea = null, $prvi_ispis = false )
        {
            //  lodanje tablice
                ...
                for( $red = 0; $red < 6; ++$red ){
                    echo '<tr>';
                        for( $stupac = 0; $stupac < 6; ++$stupac)
                        {
                            ...
                            if( $this->boje[$red][$stupac] !==-1){      //  ispisujemo samo broj (ofarban)
                            ...
                            else{       //ispisujemo polje za unos
                                echo '<input type="text" name="polje_za_unos['.$red.']['.$stupac.']" class="unos"';
                                if( $prvi_ispis)
                                    echo 'disabled';
                                echo '>';
                                if( !$prvi_ispis ){
                                    echo '<p><small>'; 
                                        print_array($this->polje_dozvoljenih($red, $stupac)) ;
                                    echo '</small></p>';
                                }
                            }
                            echo '</td>';
            ...
            echo '
                <form action="' . $_SERVER['PHP_SELF'] . '" method="post">
                    ...
                    <input type="radio" name="br_tablice" id="tablica1" value="tablica1.txt">
                        <label for="tablica1">';
                        $this->ispisi_tablicu_iz_filea( 'tablica1.txt', true);
                    echo '</label>
                    <input type="radio" name="br_tablice" id="tablica2" value="tablica2.txt">
                        <label for="tablica2">';
                        $this->ispisi_tablicu_iz_filea( 'tablica2.txt', true);
                    echo '</label>
    ...
    //  ----debug
    //echo'<hr>Javit bugove zajedno sa debug_info:<br>';
    //debug();
    //  ----------
